...
def getLogger(name: str, log_level=logging.INFO):
    ...
    if not logger.handlers:
        color_formatter = ColorFormatter(
            '%(asctime)s | %(name)-15s | %(levelname)-16s | %(threadName)-18s  | %(message)s',
            datefmt='%H:%M:%S'
        )

        plain_formatter = logging.Formatter(
            '%(asctime)s | %(name)-15s | %(levelname)-8s | %(threadName)-18s | %(message)s',
            datefmt='%H:%M:%S'
        )
        ...
        log_dir = 'server/logs'                                                # File handler (daily rotation, keep last 7 days)
